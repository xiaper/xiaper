...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  // 2018.11.06 bytedesk.,com, jackning 自定义最小化漂浮框
  //主面板最小化状态
  var layimClose, popmin = function(content){
    ...
    return layer.open({
      ...
      ,offset: 'rb'
      ,resize: false
      ,content: '<i class="iconfont icon-chat" style="font-size: 25px;"></i>'
  ...
  var anim = 'layui-anim-upbit', events = {
    ...
    ,msgbox: function(){
      ...
      // 2018/11/24 jackning, bytedesk.com 添加回调
      layui.each(call.msgbox, function(index, item){
          item && item(index);
      });
    ...
    ,find: function(){
      ...
      // 2018/11/14 jackning, bytedesk.com 添加回调
      layui.each(call.find, function(index, item){
          item && item(index);
      });
    ...
    ,groupMembers: function(othis, e){
      ...
      if(othis.data('down')){
        ...
        // 2018/11/24 jackning, bytedesk.com 添加回调
        layui.each(call.groupMembers, function(index, item){
            item && item(thisChat());
        });
